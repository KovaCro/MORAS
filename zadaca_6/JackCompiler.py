...
class Compiler:
    ...
    def read(self, filename):
        ...
        self._ofile = OutputFile(filename + ".xml")
    ...
    def _compileLet(self):
        ...
        EXIT:  Tokenizer positioned after final ';'.
        """
        self._writeXMLTag("<let-statement>\n")
    ...
    def _compileCall(self, subroutineName = None):
        """
        <subroutine-call> := 
        (<subroutine-name> '(' <expression-list> ')') | ((<class-name> | <var-name>) '.' <subroutine-name> '('<expression-list> ')')
    ...
    def _error(self, error):
        message = '%s line %d:\n  %s\n  %s' % (self._fname, self._tokenizer.lineNum(), self._tokenizer.line(), error)
